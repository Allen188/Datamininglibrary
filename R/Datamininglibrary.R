...
linda.plot.pca <- function(matrix,type,title="",mycols2= NULL,label2=NULL,ellipse.alpha=0.0,width=12,height=8){

  mat.data <- t(matrix)
  mat.data[is.na(mat.data)]=0
  com1 <- prcomp(mat.data,center = TRUE,scale. = TRUE)
  summary(com1)
  df1<-com1$x
  df1<-data.frame(df1,type)
  summ<-summary(com1)
  xlab<-paste0("PC1(",round(summ$importance[2,1]*100,2),"%)")
  ylab<-paste0("PC2(",round(summ$importance[2,2]*100,2),"%)")
  centroid <- aggregate(df1[,-ncol(df1)],by=list(df1$type), FUN = mean)
  #合并两个数据框；
  coords <- merge(df1, centroid,
                  ...
                  all.x = T,
                  suffixes = c("",".centroid"),)
  #########使用ggplot2绘制散点图#使用ggplot2绘制散点图；
  p1 <- ggplot(data = coords, aes(x = PC1, y = PC2))+
    labs(x = xlab, y = ylab)+
    ...
    stat_ellipse(aes(fill=type),
                 ...
                 alpha=ellipse.alpha,color=NA,show.legend = F)

  p2 <- p1+geom_point(data = centroid,aes(x = PC1,y = PC2),
                      colour="white",fill="DimGrey",shape = 21, size = 3.5,
                      show.legend = FALSE)+
    geom_curve(aes(xend= PC1.centroid, yend= PC2.centroid, colour = type),
               angle=150,curvature = -0.5,show.legend = FALSE)+

    geom_point(aes(fill = type),
               colour="white",shape = 21, size = 2.5,
               show.legend = FALSE)+
    #为质心（centroids）添加分组标签；
    geom_text_repel(
      data = centroid,
      aes(label = Group.1, color = Group.1),
      # colour = "black", ###标注出来的基因的颜色
      size = 3,
      segment.color = "black", show.legend = FALSE )+
    scale_x_continuous(breaks = seq(-25,30,15), limits=c(min(coords$PC1),  max(coords$PC1)))+
    scale_y_continuous(breaks = seq(-20,40,10), limits=c(min(coords$PC2), max(coords$PC2)))+
    theme_bw()
  # geom_label(data = centroid, aes(label = Group.1, fill = Group.1),
  #            size = 4,hjust=0.2, color="white",show.legend = FALSE)+theme_bw()

  #mycols2 <- c("#98d98e","#4d5aaf","#80aba9")
  if(!is.null(mycols2)){
    p2 <- p2 +  scale_fill_manual(values = mycols2,name="Group")+
      scale_colour_manual(values = mycols2)
  }

  if(!is.null(label2)){
    p2 <- p2 + geom_text(aes(label=label2,vjust = -0.8, hjust = 0.5,size=0.5),show.legend = FALSE)
  }

  ggsave(paste0(title,"_fireworkplot_pca.pdf"),plot = p2,width=width,height=height,device = NULL)
...
ge.plot.tsne <- function(data,type,title="",label=NULL){
  ...
  if(is.null(label)){
    plot(df11.tsne$Y,col=cl2, main = "t-SNE", pch = 20,cex=2,cex.axis=2,cex.lab=2)
...
ge.plot.volcano <- function(data, group1, group2, fc= 1, pvalue = 0.05, str1= "grp1",str2= "grp2",pair=F,adjust.bool=T) {
  ...
  dev.off()
  names(up)[which(names(up)== "fd")] <- "log2(fc)"
  names(down)[which(names(down)== "fd")] <- "log2(fc)"
...
#
# plot.volcano <- function(data, group1, group2, fc= 1, pvalue = 0.05, str1= "grp1",str2= "grp2",pair=F,adjust.bool=T,label2=NULL) {
#
#   fc <- as.numeric(fc)
#   pvalue <- as.numeric(pvalue)
#   len.grp1 <- length(group1)
...
#   up <- subset(df8, df8$P < pvalue & df8$fd > fc)
#   down <- subset(df8, df8$P < pvalue & df8$fd < -1*fc)
#   df8$threshold <- ifelse(df8$P < pvalue & df8$fd > fc,"up",ifelse(df8$P < pvalue & df8$fd < -1*fc,"down","non-differential"))
#
#   a <- ggplot(data = df8, aes(x = fd, y = -log10(P), colour=threshold)) +
#     geom_point(alpha=0.8, size=1) +
#     scale_color_manual(values=c(brewer.pal(11, "RdBu")[9], "#00000033",brewer.pal(9, "YlOrRd")[6])) +
#     geom_vline(xintercept=c(-1*fc, fc),lty=4,col="black",lwd=0.5) +
#     geom_hline(yintercept = -log10(pvalue),lty=4,col="black",lwd=0.5) +
#     labs(x="log2(fold change)",y=y.lab,title=paste0(str1, " vs ", str2)) +
...
#     a <- a+
#       geom_text_repel(
#         data = df8[which(row.names(df8)%in%label2),],
#         aes(label = label2),
#         size = 2,
#         box.padding = unit(0.4, "lines"),
#         point.padding = unit(0.5, "lines"), segment.color = "black", show.legend = FALSE )
#   }
#   ggsave(paste0(str1, "_", str2, "_volcano.pdf"),plot=a,width=5,height=5)
...
ge.plot.venn <- function(data.list,filename){

  if(length(data.list)<=4){
    library(VennDiagram)
    set.seed(10)
    color <- c(ge.color(length(data.list)))
    ...
    pdf(file=paste0(filename,"_VennDiagram.pdf"),width=6, height=6)
    grid.draw(venn.plot)
    dev.off()

  }else{
    print("Please enter less than 5 variables!!")
    # library(UpSetR)
    # pdf(file=paste0(filename,"_Venn_upset.pdf"),height = 8,width = 8)
    # upset(fromList(data.list),nsets = length(data.list), order.by = "freq")
    # dev.off()
    ...
    #     fl <- ""
    #     for (m in 1:nrow(com)) {
    #       all <-  intersect(all,data.list[[com[m,n]]])
    #       fl <- paste0(fl,"_",names(data.list)[[com[m,n]]])
    #     }
    #     write.csv(all,paste0(filename,"_",fl,".csv"))
    #   }
    # }
  }
...
ge.plot.heatmap <- function(data,df.lable,scale = "row", cluster_rows = T, cluster_cols = F,show_rownames = T, show_colnames = T, filename = "heatmap.pdf",width=10,height=20,fontsize_row=8){
  ...
  for (n in 1:ncol(ann_col)) {
    ...
    if(nm<11){
      set.seed(10)
      color <- c(brewer.pal(10,"Paired")[sample(1:10, nm)])
    }else{
      set.seed(10)
